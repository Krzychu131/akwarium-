...
try {
    switch ($method) {
        case 'GET':
            ...
            $allFields = ['D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'T', 'V', 'PH', 'GH', 'KH', 'NO2', 'NO3', 'CL2'];
            $completeData = [];
            ...
            foreach ($allFields as $field) {
                if (isset($data[$field]) && $data[$field] !== null && $data[$field] !== '' && $data[$field] !== -1) {
                    ...
                    // Jeśli pole nie istnieje, dodaj -1 dla D1-D6, T, V, PH, GH, KH, NO2, NO3, CL2 (aplikacja pokaże "Brak danych")
                    // lub pusty string dla S1-S6
        ...
        case 'POST':
            ...
            // WAŻNE: Zachowaj istniejące wartości D1-D6 jeśli nie są w POST
            // (aby nie tracić danych z czujników gdy aktualizujemy tylko S1-S6, T, V)
            $preservedDValues = [];
            for ($i = 1; $i <= 6; $i++) {
                $dKey = 'D' . $i;
            ...
            foreach ($postData as $key => $value) {
                ...
                $allowedKeys = ['D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'T', 'V', 'PH', 'GH', 'KH', 'NO2', 'NO3', 'CL2'];
                if (in_array($key, $allowedKeys)) {
                    if ($value === '' || $value === null) {
                        // Nie usuwaj D1-D6 nawet jeśli są puste - zachowaj istniejące wartości
                        // Nie usuwaj też pól kalibracji (PH, GH, KH, NO2, NO3, CL2)
                        if (strpos($key, 'D') !== 0 && !in_array($key, ['PH', 'GH', 'KH', 'NO2', 'NO3', 'CL2'])) {
            ...
            // Przywróć zachowane wartości D1-D6
            foreach ($preservedDValues as $key => $value) {
                $currentData[$key] = $value;
            }
            
            // WAŻNE: Jeśli akaryrym_fixed.py wysyła dane D1-D6, upewnij się że są zapisane
            // Sprawdź czy w POST są jakieś wartości D1-D6 - jeśli tak, zapisz je wszystkie
            $hasDValues = false;
            for ($i = 1; $i <= 6; $i++) {
